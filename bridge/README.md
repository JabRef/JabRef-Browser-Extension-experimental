...
The Chromium extension id is fixed by the `manifest.key` field in
`../wxt.config.ts` and matches the store-published JabRef Browser
Extension, so the native-messaging manifests work for dev builds and
...
```sh
cd bridge
./build.sh          # native-image binary into bridge/build/jabext-bridge
...
(`../src/utils/fulltextBridge.js`) covers the browser-tab fetch loop.
