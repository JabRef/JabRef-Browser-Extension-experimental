...
if [ -n "$download_version" ]; then
  ...
  cache_dir="${XDG_CACHE_HOME:-$HOME/.cache}/jabext-bridge/$download_version"
  mkdir -p "$cache_dir"
...
if [ ! -x "$bridge_path" ]; then
  echo "error: bridge binary missing at $bridge_path (run 'bridge/build.sh' or pass --download <version>)" >&2
  exit 1
fi
