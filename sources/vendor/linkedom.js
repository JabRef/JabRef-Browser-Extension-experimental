...
class EntityDecoder {
    ...
    while (offset < input.length) {
      ...
      if (valueLength === 0 && (current & BinTrieFlags.FLAG13) !== 0) {
        ...
        while (this.runConsumed < runLength) {
          ...
          const expectedChar =
            charIndexInPacked % 2 === 0 ? packedWord & 0xff : (packedWord >> 8) & 0xff;
  ...
  emitNamedEntityData(result, valueLength, consumed) {
    ...
    this.emitCodePoint(
      valueLength === 1
        ? decodeTree[result] & ~(BinTrieFlags.VALUE_LENGTH | BinTrieFlags.FLAG13)
        : decodeTree[result + 1],
      consumed,
    );
    if (valueLength === 3) {
...
let Parser$1 = class Parser {
  constructor(cbs, options = {}) {
    ...
    this.lowerCaseTagNames =
      (_a = options.lowerCaseTags) !== null && _a !== void 0 ? _a : this.htmlMode;
...
function existsOne$1(test, nodes) {
  return (Array.isArray(nodes) ? nodes : [nodes]).some(
    (node) =>
      (isTag$1(node) && test(node)) || (hasChildren(node) && existsOne$1(test, node.children)),
  );
}
/**
...
const { assign, create: create$1, defineProperties, entries, keys, setPrototypeOf } = Object;
      ...
      for (const [target, { subtree, childList, characterData }] of observer.nodes) {
        if (childList) {
          if (
            (parentNode &&
              (target === parentNode ||
                /* c8 ignore next */ (subtree && target.contains(parentNode)))) ||
            (!parentNode &&
              ((subtree &&
                (target === ownerDocument || /* c8 ignore next */ target.contains(element))) ||
...
class ParentNode extends Node$1 {
  ...
  querySelector(selectors) {
    ...
    while (next !== end) {
      ...
      next =
        next.nodeType === ELEMENT_NODE && next.localName === "template" ? next[END] : next[NEXT];
    }
    return null;
  }
...
var uhyphen = (camel) =>
  camel.replace(/(([A-Z0-9])([A-Z0-9][a-z]))|(([a-z0-9]+)([A-Z]))/g, "$2$5-$3$6").toLowerCase();
...
let Element$1 = class Element extends ParentNode {
  ...
  get textContent() {
    ...
    while (next !== end) {
      ...
      if (nodeType === TEXT_NODE || nodeType === CDATA_SECTION_NODE) text.push(next.textContent);
      next = next[NEXT];
    }
    return text.join("");
  }
